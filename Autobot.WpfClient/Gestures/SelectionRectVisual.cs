...
namespace Autobot.WpfClient.Gestures
{
    ...
    internal class SelectionRectVisual : FrameworkElement
    {
        ...
        public SelectionRectVisual(Point firstPointP, Point secondPointP, double zoomP)
        {
            ...
            this._horizontalDashBrush = drawingBrush;
            this._verticalDashBrush = drawingBrush.Clone();
            this._verticalDashBrush.Transform = new RotateTransform(90);

            this._firstPoint = firstPointP;
            this._secondPoint = secondPointP;
            this._zoom = zoomP;
            this._visualForRect = new DrawingVisual();
            this.AddVisualChild(this._visualForRect);
            this.AddLogicalChild(this._visualForRect);      
        }
        ...
        void DrawOnTheContext()
        {
            ...
            if (topLeftCorner.X != bottomRightCorner.X || topLeftCorner.Y != bottomRightCorner.Y)
            {
                using (DrawingContext drawingContext = this._visualForRect.RenderOpen())
                {
                    ...
                    this._horizontalDashBrush.Viewport = new Rect(0, 0, _dashRepeatLength, _dashRepeatLength);
                    drawingContext.DrawRectangle(this._horizontalDashBrush, null, new Rect(topLeftCorner - lineOffset, topRightCorner + lineOffset));
                    drawingContext.DrawRectangle(this._horizontalDashBrush, null, new Rect(bottomLeftCorner - lineOffset, bottomRightCorner + lineOffset));

                    // Draw the two vertical lines.
                    this._verticalDashBrush.Viewport = this._horizontalDashBrush.Viewport;
                    drawingContext.DrawRectangle(this._verticalDashBrush, null, new Rect(topLeftCorner - lineOffset, bottomLeftCorner + lineOffset));
                    drawingContext.DrawRectangle(this._verticalDashBrush, null, new Rect(topRightCorner - lineOffset, bottomRightCorner + lineOffset));
        ...
        {
            get { return new Rect(this._firstPoint, this.SecondPoint); }
        }
    }
}
