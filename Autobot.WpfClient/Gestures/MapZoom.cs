...
namespace Autobot.WpfClient.Gestures
{
    ...
    class MapZoom : Animatable
    {
        ...
        public MapZoom(FrameworkElement target)
        {
            ...
            Keyboard.AddKeyUpHandler(this._container, new KeyEventHandler(this.OnKeyUp));

            this._container.Focusable = true;
            ...
            this._zoom = this._newZoom = this._scale.ScaleX;
        ...
        {
            if (this._offset.X != this._translate.X || this._offset.Y != this._translate.Y)
            {
        ...
        {
            if (this._zoom != this._scale.ScaleX)
            {
                this._zoom = this._scale.ScaleX;
            }
        }
        ...
        void OnKeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                this._container.Cursor = Cursors.SizeAll;
            }
        }
        ...
        {
            this._mouse = this._container.PointToScreen(e.GetPosition(this._container));
            this._onTarget = e.GetPosition(this._target);
            ...
            if (this._onTarget.X < 50 / this._zoom) this._onTarget.X = 0;
            if (this._onTarget.Y < 50 / this._zoom) this._onTarget.Y = 0;
            if (this._onTarget.X + 50 / this._zoom > this._target.ActualWidth) this._onTarget.X = this._target.ActualWidth;
            if (this._onTarget.Y + 50 / this._zoom > this._target.ActualHeight) this._onTarget.Y = this._target.ActualHeight;
        }
        ...
        public void OnMouseWheel(object sender, MouseWheelEventArgs e)
        {
            ...
            {
                this.HandleZoom((double)e.Delta / (double)Mouse.MouseWheelDeltaForOneLine);
                e.Handled = true;
            }
        }
        ...
        void OnZoomChanged()
        {
            this._scale.ScaleX = this._scale.ScaleY = this._zoom;
        ...
        public Size ContainerSize
        {
            get
            {
                IScrollInfo si = this._target as IScrollInfo;
                if (si != null)
                {
        ...
        void KeepPositionStable()
        {
            ...
            try
            {
                moved = this._target.PointFromScreen(this._mouse);
            }
            catch (Exception)
            {
                return;
            ...
            double x = this._translate.X + (delta.X * this._zoom);
            double y = this._translate.Y + (delta.Y * this._zoom);

            Size containerSize = this.ContainerSize;
            double width = containerSize.Width;
            double height = containerSize.Height;
            
            double right = (this._target.ActualWidth * this._zoom) + x;
            if (right < width && x < 0)
            {
                x += (width - right);
            }

            double bottom = (this._target.ActualHeight * this._zoom) + y;
        ...
        void KeepRectStable()
        {
            ...
            Rect cr = this._target.TransformToAncestor(this._container).TransformBounds(this._targetRect);
            // Keep it centered.
            Size containerSize = this.ContainerSize;
            double width = containerSize.Width;
            double height = containerSize.Height;
            ...
            double dx = this._translate.X + cx - cr.X;
            double dy = this._translate.Y + cy - cr.Y;
        ...
        void Translate(double x, double y)
        {
            ...
            if (si != null)
            {
                ...
                if (this._target.ActualWidth <= s.Width && this._target.ActualHeight <= s.Height)
                {
                    x = y = 0;
                }
            }
            ...
            this._translate.X = this._offset.X = x;
            this._translate.Y = this._offset.Y = y;
        ...
        void StopAnimations()
        {
            ...
            Point t = new Point(this._translate.X, this._translate.Y);
        ...
        public void ZoomToRect(Rect r)
        {
            ...
            Rect cr = this._target.TransformToAncestor(this._container).TransformBounds(r);
            ...
            this._targetRect = r;
            this._startTime = 0;
            this.AnimateZoom(ZoomToRectProperty, oldZoom, zoom, new Duration(TimeSpan.FromMilliseconds(_defaultZoomTime)));
        ...
        {
            this.StopAnimations();
            this._scale.ScaleX = this._scale.ScaleY = this._zoom = this._newZoom = 1;
            this._translate.X = this._offset.X = 0;
            this._translate.Y = this._offset.Y = 0;
            this._startTime = 0;
            this.OnZoomChanged();
            // focus rectangles may need to be repainted.
            this._target.InvalidateVisual();
        ...
        void HandleZoom(double clicks)
        {
            ...
            if (sameSign && this._startTime != 0 && this._startTime + this._zoomTime > tick)
            {
                // then make the time cumulative so you get nice smooth animation when you flick the wheel.
                this._zoomTime += (this._startTime + this._zoomTime - tick);
            ...
            this.AnimateZoom(ZoomToPointProperty, oldZoom, this._newZoom, new Duration(TimeSpan.FromMilliseconds(this._zoomTime)));
        }
        ...
        public Rect ScrollIntoView(FrameworkElement e)
        {
            ...
            rect = e.TransformToAncestor(this._container).TransformBounds(rect);
            rect.Inflate(margin, margin);
            this.ScrollIntoView(rect, new Duration(TimeSpan.FromMilliseconds(_defaultZoomTime)), true);
            return rect;
        }
        ...
        public void ScrollIntoView(FrameworkElement[] elements)
        {
            ...
            this.ScrollIntoView(result, new Duration(TimeSpan.FromMilliseconds(_defaultZoomTime)), true);
        }
        ...
        void ScrollIntoView(Rect rect, Duration duration, bool zoom)
        {
            ...
            {
                Rect tr = this._container.TransformToDescendant(this._target).TransformBounds(rect);
                if (zoom)
                {
                    this.ZoomToRect(tr);
                    return;
            ...
            if (delta.X != 0 || delta.Y != 0)
            {
                ...
                Point startPos = new Point(this._translate.X, this._translate.Y);
