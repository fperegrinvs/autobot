namespace Autotob.Brick
{
	...
	public abstract class BrickCommand
	{
		...
		public void Append(bool b){
			if(b)
				this.dataArr.Add(0x01);
			else
				this.dataArr.Add(0x00);
		}
		...
		public void Append(String s){
			for(int i = 0; i < s.Length; i++){
				this.dataArr.Add((byte)(s[i]));
			}
			this.dataArr.Add((byte)0x00);
		}
		...
		public void Append(byte [] data, int offset){
			this.Append(data,offset,data.Length);
		}
		...
		public void Append(byte [] data, int offset , int length){
			for(int i = 0; i < length; i++){
				this.dataArr.Add(data[i+offset]);
			}
		}
		...
		/// </value>
		public byte[] Data{
			get{return  this.dataArr.ToArray();}
		}
	...
	public class BrickReply
	{
		...
		/// </value>
		public byte[] Data{
			get{return (byte[]) this.dataArray.Clone();}
		}
		...
		public string GetString(byte offset){
			byte size = 0;
			while(offset + size < this.dataArray.Length && this.dataArray[offset+size] != 0){
				size++;
			}
			if(offset+size > this.dataArray.Length){
				return "";
			}
		...
		public string GetString(byte offset, byte length){
			return  System.Text.ASCIIEncoding.ASCII.GetString(this.dataArray, offset,length);
		}
		...
		public byte[] GetData(int offset){
			...
			if(offset <= this.dataArray.Length){
				int newSize = this.dataArray.Length-offset; 
				a = new byte[newSize];
			    for(int i = 0; i < newSize; i++){
					a[i] = this.dataArray[i+offset];
				}    
	        }
			return a;
		}
        ...
        public UInt16 GetUInt16(int offset){
			return BitConverter.ToUInt16(this.dataArray,offset); 
		}
		...
		public Int16 GetInt16(int offset){
			return BitConverter.ToInt16(this.dataArray,offset); 
		}
		...
		public UInt32 GetUInt32(int offset){
			return BitConverter.ToUInt32(this.dataArray,offset);
		} 
		...
		public Int32 GetInt32(int offset){
			return BitConverter.ToInt32(this.dataArray,offset);
		}
		...
		public float GetFloat(int offset){
			return BitConverter.ToSingle(this.dataArray,offset);
		}
