namespace Autotob.Brick.EV3
{
    ...
    /// <summary>
		...
		public class Output{
			...
			/// Initializes a new instance of the <see cref="Output"/> class.
			/// </summary>
			public Output(): this(OutputBitfield.OutA, DaisyChainLayer.EV3){
			}
		...
		public class MotorSync : MotorBase{
			...
			public void On(sbyte speed, Int16 turnRatio, bool reply){
				this.StepSync(speed, turnRatio,0, reply);
			}
			...
			{
				this.StepSync(speed,turnRatio,degrees, false, reply);
			}
		}
		...
		public class Vehicle: IVehicle{
			...
			public void Backward(sbyte speed, UInt32 degrees, bool reply = false){
				this.Move((sbyte)-speed,degrees,reply);
			}
			...
			public void TurnRightForward(sbyte speed, sbyte turnPercent, bool reply = false){
				if(this.leftPort < this.rightPort){
					this.HandleRightForward(speed, turnPercent, 0, reply);	
				}
				else{
					this.HandleLeftForward(speed,turnPercent, 0, reply);	
				}
			}
			...
			public void TurnRightForward(sbyte speed, sbyte turnPercent, UInt32 degrees, bool reply= false){
				if(this.leftPort < this.rightPort){
					this.HandleRightForward(speed, turnPercent, degrees, reply);	
				}
				else{
					this.HandleLeftForward(speed,turnPercent, degrees, reply);	
				}
			}
			...
			public void TurnLeftForward(sbyte speed, sbyte turnPercent, bool reply){
				this.TurnLeftForward(speed,turnPercent, 0, reply);
			}
			...
			private void HandleLeftForward(sbyte speed, sbyte turnPercent, UInt32 degrees, bool reply){
				...
				if(this.ReverseLeft && !this.ReverseRight){
					this.motorSync.On(speed, (short) ((short)-200+(short)turnPercent), degrees, reply);
				}
				if(this.ReverseLeft && this.ReverseRight){
			...
			private void HandleRightForward(sbyte speed, sbyte turnPercent, UInt32 degrees, bool reply){
				if(!this.ReverseLeft && !this.ReverseRight){
					this.motorSync.On(speed, (short) turnPercent, reply);
				}
				if(!this.ReverseLeft && this.ReverseRight){
					this.motorSync.On(speed, (short) ((short)200- (short)turnPercent), degrees, reply);
				}
			...
			private void Move(sbyte speed, UInt32 degrees, bool reply){
				...
				if(!this.ReverseLeft && this.ReverseRight){
					this.motorSync.On(speed, (short) 200, degrees, reply);
				}
				if(this.ReverseLeft && !this.ReverseRight){
					this.motorSync.On(speed, -200, degrees, reply);
				}
