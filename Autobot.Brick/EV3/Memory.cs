    ...
    /// <summary>
		...
		public class MemoryArray<TValue>{
			...
			private void Fill(){
				...
				command.Append(this.Handle, ConstantParameterType.Value);
			...
			internal void CreateArray(Int32 size){
				...
				if (typeof(TValue) == typeof(byte)){
					this.CreateArray(size, MemorySubCodes.Create8);	
				}
			    
			    if (typeof(TValue) == typeof(Int16)){
			    	this.CreateArray(size, MemorySubCodes.Create16);
			    }
			    
			    if (typeof(TValue) == typeof(Int32)){
			    	this.CreateArray(size, MemorySubCodes.Create32);
			    }
			    
			    if(typeof(TValue) == typeof(float)){
			    	this.CreateArray(size, MemorySubCodes.CreateTEF);
			    }
			    //Init(size);
			}
			...
			{
				this.Write(data, 0, 0, data.Length);
			}
			...
			{
				this.Write(data, memoryIndex, 0, data.Length);
			}
			...
			public void Write (TValue[] data, Int32 memoryIndex, Int32 offset, Int32 length)
			{
				for(int i = 0; i < length; i++){
					this.Write(data[i+offset], memoryIndex);
					memoryIndex++;
				}		
			}
			...
			public TValue[] Read (Int32 memoryIndex, Int32 length)
			{
				...
				for(int i = 0; i < length; i++){
					valueRead[i] = this.ReadSingleValue(memoryIndex);
					memoryIndex++;
				}
				return valueRead;
			}
