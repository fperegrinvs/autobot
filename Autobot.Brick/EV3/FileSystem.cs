namespace Autotob.Brick.EV3
{
	...
	public class BrickFile: IBrickFile, IComparable<BrickFile>{
		...
		public BrickFile (string name, UInt32 size, string path){
			...
			this.fileSize = size;
			this.extension = System.IO.Path.GetExtension(name).ToLower();
			this.ev3FileType = this.StringToFileType(this.extension);
			this.Path =  path;
		}
		...
		/// </value>
		public bool IsEmpty{
			get{return !Convert.ToBoolean(this.fileSize);}
		}
	...
	public class FolderStructure: IComparable<FolderStructure>{
		...
		public FolderStructure(string path, bool isBrowsable){
			this.Path = path;
			this.FileList = new List<BrickFile>();
			this.FolderList = new List<FolderStructure>();
    	...
    	{
	       return this.RecursiveRunThrough(this);
    	}
		...
		private IEnumerable<FolderStructure> RecursiveRunThrough(FolderStructure structure)
	    {
	      ...
	      foreach (var child in structure.FolderList) {
	        foreach (var f in this.RecursiveRunThrough(child))
	        {
	          yield return f;
	        }
	      }
	    ...
	    /// <summary>
	    /// Returns a <see cref="System.String"/> that represents the current <see cref="FolderStructure"/>.
	    /// </summary>
	...
	public class FilSystem
	{
		...
		private void WriteCompleteArray(string fileName, byte[] data){
			...
			try{
				handle = this.BeginWrite((UInt32) data.Length, fileName);
				while (bytesWritten < bytesToWrite) {
					if ((bytesToWrite - bytesWritten) >= MaxBytesToWrite) {
						this.ContinueWrite (handle, data, bytesWritten, MaxBytesToWrite);
						bytesWritten += MaxBytesToWrite;
					}
					else{
						this.ContinueWrite (handle, data, bytesWritten, (bytesToWrite - bytesWritten));
		...
		private byte[] ReadWholeFile(string fileName){
			...
			byte handle = 0;
			UInt32 fileSize = 0;
			dataRead = this.BeginRead (MaxBytestoRead, fileName, out handle, out fileSize);
			...
			while(bytesRead < fileSize){
				if( (fileSize - bytesRead) >= MaxBytestoRead){
					dataRead = this.ContinueRead(handle, MaxBytestoRead);
					Array.Copy (dataRead, 0, fileData, bytesRead, dataRead.Length);
					bytesRead = bytesRead + MaxBytestoRead;
				}
				else{
					dataRead = this.ContinueRead(handle,(UInt16) (fileSize - bytesRead));
					Array.Copy (dataRead, 0, fileData, bytesRead, dataRead.Length);
		...
		private byte[] OpenHandles(){
			var reply = this.connection.SendAndReceive (new Command (SystemCommand.ListOpenHandles, 108, true));
		...
		private string ListFiles(string path, UInt16 length){
			...
			var reply = this.connection.Receive();
		...
		public void GetFolderInfo (string path, out BrickFile[] files, out string[] subFoldersNames)
		{
			...
			string list = this.ListFiles(path,(UInt16) MaxBytesInFileList);
		...
		public FolderStructure GetFolderStructure(string path)
		{
			...
			try{
				BrickFile[] files;
				string[] folders;
				this.GetFolderInfo(path, out files, out folders);
				...
				for(int i = 0; i < folders.Length; i++){
					...
					folderStructure.FolderList.Add(this.GetFolderStructure(newPath));
				}
			}
			catch{
		...
		public void ReadFile( BrickFile file, string destinationFileName){
			this.ReadFile (file.Name, destinationFileName);
		}
		...
		public void ReadFile(string brickFileName, string destinationFileName){
			...
			byte[] fileData = this.ReadWholeFile (brickFileName);
		...
		public void WriteFile(string fileToWrite, string brickFileName){
			...
			fileStream.Close();
			this.WriteCompleteArray (brickFileName, fileData);
		}
