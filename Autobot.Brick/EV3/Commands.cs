namespace Autotob.Brick.EV3
{
	...
	public class Command: BrickCommand{
		...
		public Command(byte [] data){
			...
			for(int i = 0; i < data.Length; i++){
				this.dataArr.Add(data[i]);
			}
			this.sequenceNumber = (UInt16)(0x0000 | this.dataArr[0] | (this.dataArr[1] << 2));
			try{
				this.commandType = (CommandType) (data[2] & 0x7f);
				if(this.commandType == CommandType.SystemCommand){
					this.systemCommand = (SystemCommand) data[3];
				}
				else{
			...
			this.replyRequired = !Convert.ToBoolean(data[2]&0x80);
		}
		...
		public Command(SystemCommand systemCommand, UInt16 sequenceNumber, bool reply)
		{
			...
			if (reply){
				...
				this.dataArr.Add((byte)this.commandType);
			}
			else{
				this.replyRequired = false;
				this.dataArr.Add((byte)((byte) this.commandType | 0x80));
			}
			this.dataArr.Add((byte)systemCommand);
		}
		...
		{
			this.Append((byte) type, ParameterFormat.Short);
		}
		...
		{
			this.Append((byte) mode, ParameterFormat.Short);
		}
		...
		public void Append(FileSubCodes code){
			this.Append((sbyte) code, ParameterFormat.Short);	
		}
		...
		public void Append(DaisyChainLayer chain){
			this.Append((sbyte) chain, ParameterFormat.Short);
		}
		...
		{
			this.Append((sbyte) subCode, ParameterFormat.Short);
		}
		...
		{
			this.Append((sbyte) port, ParameterFormat.Short);
		}
		...
		{
			this.Append((sbyte) bitField, ParameterFormat.Short);
		}
		...
		{
			this.Append(type, FollowType.OneByte);
			this.Append (value);
		}
		...
		public void Append(Int16 value , ConstantParameterType type){
			this.Append(type, FollowType.TwoBytes);
			this.Append (value);
		}
		...
		public void Append(Int32 value, ConstantParameterType type){
			this.Append(type,  FollowType.FourBytes);
			this.Append(value);
		}
		...
		public void Append(string s, ConstantParameterType type){
			this.Append(type,  FollowType.TerminatedString2);
			this.Append (s);
		}
		...
		{
			this.Append(scope, type, FollowType.OneByte);
			this.Append (value);
		}
		...
		public void Append(Int16 value , VariableScope scope, VariableType type){
			this.Append(scope, type, FollowType.TwoBytes);
			this.Append (value);
		}
		...
		public void Append(Int32 value, VariableScope scope, VariableType type){
			this.Append(scope, type, FollowType.FourBytes);
			this.Append(value);
		}
		...
		public void Append(string s, VariableScope scope, VariableType type){
			this.Append(scope, type, FollowType.TerminatedString2);
			this.Append (s);
		}
	...
	public class Reply: BrickReply
	{
		...
		public bool HasError{
			get{
				CommandType type = (CommandType)this.dataArray[2];
		...
		internal ErrorType ErrorType{
			get{
				return Error.ToErrorType(this.ErrorCode);
			}
		}
		...
		public byte ErrorCode {
			get {
				if (this.HasError) {
					if(this.CommandType == CommandType.SystemReplyWithError){
						if(this.dataArray.Length >=5){
							byte error = this.dataArray[4];
							if(Enum.IsDefined(typeof(MonoBrick.EV3.BrickError),(int) error)){
								return error;
							}
						}
					}
		...
		public SystemCommand SystemCommandType{
			get{
				if(this.CommandType == CommandType.SystemReply){
					return (SystemCommand) this.dataArray[3];
				}
		...
		/// <value>The type of the command.</value>
		public CommandType CommandType {
			get{return (CommandType) this.dataArray[2];}
		}
		...
		/// <value>The sequence number.</value>
		public UInt16 SequenceNumber {
			get{return (UInt16)(0x0000 | this.dataArray[0] | (this.dataArray[1] << 2));}
		}
		...
		public string CommandTypeAsString{
			get{return BrickCommand.AddSpacesToString(this.CommandType.ToString()); }
		} 

		internal void print(){
			Console.WriteLine("Command: " + this.CommandType.ToString());
			Console.WriteLine("Length: " + this.Length);
			Console.WriteLine("Errorcode: " + this.ErrorCode);
			for(int i = 0; i < this.Length; i++){
				Console.WriteLine("Reply["+i+"]: " + this.dataArray[i].ToString("X"));
			}
